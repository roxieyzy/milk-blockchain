<template>
  <el-container class="saleHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
...
export default {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: '/saleHome'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/saleHome' && this.$route.path !== '/saleHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/saleHome', name: '销售商首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/saleHome', name: '销售商首页'})
      this.$store.commit('set_active_index', '/saleHome')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/saleHome' && to.name === undefined) {
          return
        }
