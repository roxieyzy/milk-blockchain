<template>
  <el-container class="produceHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
...
<script>
// @ is an alias to /src
import produceSlider from '@/components/ProduceSlider.vue'
import navTop from '@/components/Header.vue'
export default {
  name: 'ProduceHome',
  components: {
    produceSlider, navTop
  },
  data () {
    return {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: '/produceHome'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/produceHome' && this.$route.path !== '/produceHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/produceHome', name: '生产商首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/produceHome', name: '生产商首页'})
      this.$store.commit('set_active_index', '/produceHome')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/produceHome' && to.name === undefined) {
          return
        }
