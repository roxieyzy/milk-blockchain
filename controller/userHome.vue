<template>
  <el-container class="userHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
...
export default {
  ...
  mounted () {
    ...
    if (this.$route.path !== '/userHome' && this.$route.path !== '/userHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/userHome', name: '用户首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/userHome', name: '用户首页'})
      this.$store.commit('set_active_index', '/userHome')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/userHome' && to.name === undefined) {
          return
        }
