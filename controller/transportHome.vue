<template>
  <el-container class="transportHome" direction="vertical">
    <!-- header -->
    <nav-top></nav-top>
    <!-- 下边 -->
    <el-container class="home-content" direction="vertical">
     <el-row>
      <el-col :span="4">
        <!-- 左侧导航栏 -->
        <!-- <slider></slider> -->
				<transport-slider></transport-slider>
      </el-col>
...
<script>
// @ is an alias to /src
import transportSlider from '@/components/TransportSlider.vue'
import navTop from '@/components/Header.vue'
export default {
  name: 'TransportHome',
  components: {
    transportSlider, navTop
  },
  data () {
    return {
    ...
    tabRemove (targetName) {
      ...
      if (this.activeIndex === targetName) {
        ...
        if (this.openTab && this.openTab.length >= 1) {
          ...
          this.$router.push({path: '/transportHome'})
        }
      }
    }
  },
  mounted () {
    ...
    if (this.$route.path !== '/transportHome' && this.$route.path !== '/transportHome') {
      console.log('1')
      this.$store.commit('add_tabs', {route: '/transportHome', name: '运输商首页'})
      ...
      console.log('2')
      this.$store.commit('add_tabs', {route: '/transportHome', name: '运输商首页'})
      this.$store.commit('set_active_index', '/transportHome')
      // this.$router.push('/')
  ...
  watch: {
    '$route' (to, from) {
      ...
      if (!flag) {
        ...
        if (to.path === '/transportHome' && to.name === undefined) {
          return
        }
